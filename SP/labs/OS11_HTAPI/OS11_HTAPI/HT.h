...
namespace ht    // HT API
{
	...
	extern "C" SP04_HTAPI_API  struct HtHandle    // блок управления HT
	{
	...
	extern "C" SP04_HTAPI_API  HtHandle * create   //  создать HT             
	(
	...
	extern "C" SP04_HTAPI_API  HtHandle * open     //  открыть HT             
	(
		const wchar_t* fileName,         // имя файла
		bool isMapFile = false			// true если открыть fileMapping; false если открыть файл; по умолчанию false
	); 	// != NULL успешное завершение  

	extern "C" SP04_HTAPI_API  BOOL snap         // выполнить Snapshot
	(
		HtHandle * htHandle           // управление HT (File, FileMapping)
	);

	extern "C" SP04_HTAPI_API  BOOL close        // snap и закрыть HT  и  очистить htHandle
	(
		const HtHandle * htHandle           // управление HT (File, FileMapping)
	);	//  == TRUE успешное завершение   


	extern "C" SP04_HTAPI_API  BOOL insert      // добавить элемент в хранилище
	(
	...
	extern "C" SP04_HTAPI_API BOOL removeOne      // удалить элемент в хранилище
	(
		HtHandle * htHandle,            // управление HT (ключ)
		const Element * element              // элемент 
	);	//  == TRUE успешное завершение 

	extern "C" SP04_HTAPI_API Element * get     //  читать элемент в хранилище
	(
		HtHandle * htHandle,            // управление HT
		const Element * element              // элемент 
	); 	//  != NULL успешное завершение 
	...
	extern "C" SP04_HTAPI_API  BOOL update     //  именить элемент в хранилище
	(
		HtHandle * htHandle,            // управление HT
	...
	extern "C" SP04_HTAPI_API  const char* getLastError  // получить сообщение о последней ошибке
	(
		const HtHandle * htHandle                         // управление HT
	);

	extern "C" SP04_HTAPI_API  void print                               // распечатать элемент 
	(
		const Element * element              // элемент 
	);
